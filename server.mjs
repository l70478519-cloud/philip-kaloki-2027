...
app.get('/api/health',async(_req,res)=>{const{error}=await supabase.from('campaign_content').select('id').limit(1);res.status(error?503:200).json({status:error?'error':'ok',service:'philip-kaloki-website-api',storage:'supabase-postgresql',phase:'admin-cms-17'})})
...
function crud(table,kind){app.get(`/api/admin/${kind}`,adminOnly,async(_req,res)=>{const{data,error}=await supabase.from(table).select('*').order('created_at',{ascending:false});if(error)return res.status(500).json({error:error.message});res.json(data||[])});app.post(`/api/admin/${kind}`,adminOnly,async(req,res)=>{const p=clean(req.body);if('published'in p)p.published=String(p.published)==='true';const{data,error}=await supabase.from(table).insert(p).select().single();if(error)return res.status(500).json({error:error.message});await audit(`${kind}_created`,table,data.id);res.status(201).json(data)});app.put(`/api/admin/${kind}/:id`,adminOnly,async(req,res)=>{const p=clean(req.body);if('published'in p)p.published=String(p.published)==='true';p.updated_at=new Date().toISOString();const{data,error}=await supabase.from(table).update(p).eq('id',req.params.id).select().single();if(error)return res.status(500).json({error:error.message});await audit(`${kind}_updated`,table,data.id);res.json(data)});app.delete(`/api/admin/${kind}/:id`,adminOnly,async(req,res)=>{const{error}=await supabase.from(table).delete().eq('id',req.params.id);if(error)return res.status(500).json({error:error.message});await audit(`${kind}_deleted`,table,req.params.id);res.json({ok:true})})}
