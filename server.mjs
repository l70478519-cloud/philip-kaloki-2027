...
app.get('/api/homepage-slides',async(_req,res)=>{const{data,error}=await supabase.from('homepage_slides').select('*').eq('is_active',true).order('sort_order',{ascending:true}).order('created_at',{ascending:true});res.status(error?500:200).json(error?{error:error.message}:data||[])})
